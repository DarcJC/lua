...
/*
* The code below can be used to print the opcodes and other information.
*/

#define UPVALNAME(x) ((f->upvalues[x].name) ? getstr(f->upvalues[x].name) : "-")
#define VOID(p) ((const void*)(p))
#define eventname(i) "metamethod"
...
static void PrintCode(const Proto* f)
{
 ...
 for (pc=0; pc<n; pc++)
 {
  ...
  switch (o)
  {
   ...
   case OP_NEWTABLE:
    printf("%d %d %d",a,b,c);
    printf(COMMENT "%d",c+EXTRAARGGET);
    break;
   case OP_SELF:
    printf("%d %d %d%s",a,b,c,ISK);
    if (isk) { printf(COMMENT); PrintConstant(f,c); }
   ...
   case OP_SETLIST:
    printf("%d %d %d%s",a,b,c,ISK);
    if (isk) printf(COMMENT "%d",c+EXTRAARGGET);
    break;
   case OP_CLOSURE:
    printf("%d %d",a,bx);
